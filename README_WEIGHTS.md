...
> **Personal note**: When loading the BF16 weights instead of FP8, omit the `quantization_config` field from `config.json` entirely (or set it to `null`). Leaving a stale FP8 config in place while loading BF16 checkpoints can cause silent dtype mismatches in some inference frameworks.
